...
| --- | --- | --- |
| `DASH_PASSWORD` | yes | The password you already use to open `/dash/` |
| `BREVO_API_KEY` | for leads | Brevo → SMTP & API → API keys |
| `CALCOM_API_KEY` | for calls | Cal.com → Settings → Developer → API keys |
| `STRIPE_API_KEY` | for revenue | Stripe → Developers → API keys (a restricted key with **read** on subscriptions and charges is enough) |
